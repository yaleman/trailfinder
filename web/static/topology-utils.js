...
async function loadAndRenderTopology(containerId, options = {}) {
    try {
        console.debug('Loading topology data...');
        const response = await fetch('/api/topology');
        const topology = await response.json();

        console.debug('Topology loaded, rendering...');
...
// Used to identify topolgy nodes
const tooltip_classname = "topology_tooltip";

/**
 * Render the network topology with D3.js force simulation
 */
function renderNetworkTopology(topology, containerId, options = {}) {
    console.debug('Starting D3.js topology render');
    ...
    console.debug('D3.js topology render complete');
    return controls;
}
